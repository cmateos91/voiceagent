...
import { getActiveProviderConfig } from './provider-config.js';
...
{"type":"command","message":"Abriendo editor de texto.","command":"xdg-open /etc/hostname"}

Usuario: ábreme un editor de texto
{"type":"command","message":"Abriendo editor de texto.","command":"xdg-open /etc/hostname"}

Usuario: abre el navegador
{"type":"command","message":"Abriendo navegador.","command":"xdg-open https://google.com"}
...
Usuario: abre el bloc de notas
{"type":"command","message":"Abriendo Bloc de notas.","command":"notepad"}

Usuario: ábreme un editor de texto
{"type":"command","message":"Abriendo editor de texto.","command":"notepad"}

Usuario: abre el navegador
{"type":"command","message":"Abriendo navegador.","command":"start https://google.com"}
...
function BASE_RULES(scopeRule = '') {
  return (
    'Eres un agente local de PC.\n' +
    'Responde SIEMPRE en JSON válido: {"type":"reply","message":"..."} o {"type":"command","message":"...","command":"..."}.\n' +
    'CRÍTICO: Responde ÚNICAMENTE con JSON válido. NUNCA escribas texto fuera del JSON. NUNCA uses backticks ni markdown en el campo message. Si necesitas varios pasos, elige el más importante como comando.\n' +
    ...
    'El comando debe ser una sola línea ejecutable.\n' +
    scopeRule +
    'No escribas nada fuera del JSON.\n'
  );
}

function CONTEXT_BLOCK({ availableDirsText, memory, platform, trimmedMemory, accessCfg }) {
  const scopeInfo =
    accessCfg.mode === 'free'
      ? '- Acceso: TODO el sistema de archivos. Puedes usar rutas absolutas.\n'
      : accessCfg.mode === 'allowlist'
        ? `- Acceso: rutas permitidas: ${(Array.isArray(accessCfg.allowedPaths) ? accessCfg.allowedPaths : []).join(', ')}\n`
        : `- Acceso: solo directorio de trabajo (${EXEC_WORKDIR})\n`;
  return (
    `Contexto actual:\n` +
...
function isSmallModel(providerCfg) {
  if (providerCfg.provider === 'ollama') return true;
  return false;
}
...
export async function askModel(history, lastUserMessage, memoryContext = '', onToken, context = {}) {
  ...
  const fewShotExamples = platform === 'win32' ? FEW_SHOT_WINDOWS : FEW_SHOT_LINUX;
  const small = isSmallModel(providerCfg);
  const fewShot = small ? fewShotExamples + FEW_SHOT_COMMON : FEW_SHOT_COMMON;
  const scopeRule =
    accessCfg.mode === 'free'
      ? '- Puedes usar rutas absolutas en cualquier parte del sistema. Para buscar archivos usa: find /home -name "archivo" 2>/dev/null\n'
      : '';
  const detailedRules = small
    ? '- Nunca uses pipes (|), redirecciones (>, >>) ni operadores (&&, ;).\n' +
      '- Para "bloc de notas" o "editor de texto": usa xdg-open /etc/hostname en linux. Genera UN SOLO comando ejecutable. NUNCA expliques pasos, NUNCA uses backticks en message.\n' +
      '- Para abrir el navegador: xdg-open https://google.com.\n' +
      '- Para abrir una carpeta: xdg-open <ruta>.\n' +
      '- En windows usa start o notepad según el caso.\n' +
      '- Nunca uses apps específicas (gedit, kate) salvo mención explícita.\n' +
      '- Solo comandos simples de una palabra + argumentos.\n'
    : '- No uses shell operators (|, >, &&, ;) — use simple commands only.\n';
  const trimmedMemory = memoryContext ? memoryContext.split('\n').slice(-5).join('\n') : '';

  // SYSTEM PROMPT STRUCTURE:
  // [1] BASE_RULES (always)
  // [2] detailedRules (full for ollama, minimal for api providers)
  // [3] fewShot (full for ollama, FEW_SHOT_COMMON only for api)
  // [4] CONTEXT_BLOCK (always)
  const system = {
    role: 'system',
    content: BASE_RULES(scopeRule) + detailedRules + '\nEjemplos de uso:\n' + fewShot + '\n' + CONTEXT_BLOCK({
      availableDirsText,
      memory,
      platform,
      trimmedMemory,
...
export async function summarizeExecution(lastUserMessage, execution, options = {}) {
  ...
  const summaryPrompt =
    `Responde en JSON: {"type":"reply","message":"..."}\n` +
    `El mensaje debe ser UNA frase corta (máx 10 palabras) confirmando qué se hizo. ` +
    `Sin explicaciones, sin detalles técnicos.\n` +
    `Acción realizada: ${lastUserMessage}\n` +
    `Resultado: ${execution.ok ? 'exitoso' : 'falló'}\n` +
  ...
  try {
    ...
    if (parsed?.message) {
      summary = String(parsed.message).trim();
    }
  } catch {
    ...
    if (start >= 0 && end > start) {
      try {
        const parsed = JSON.parse(summary.slice(start, end + 1));
        if (parsed?.message) {
          summary = String(parsed.message).trim();
        }
      } catch {
        // Keep raw summary fallback.
      }
    }
  }
