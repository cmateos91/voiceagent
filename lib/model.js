...
{"type":"command","message":"Abriendo editor de texto.","command":"xdg-open /tmp/nuevo.txt"}
...
function CONTEXT_BLOCK({ availableDirsText, memory, platform, trimmedMemory }) {
  return (
    `Contexto actual:\n` +
...
export async function askModel(history, lastUserMessage, memoryContext = '', onToken, context = {}) {
  ...
  const detailedRules = small
    ? '- Nunca uses pipes (|), redirecciones (>, >>) ni operadores (&&, ;).\n' +
      '- Para abrir el bloc de notas en linux: xdg-open /tmp/nuevo.txt.\n' +
  ...
  const system = {
    role: 'system',
    content: BASE_RULES + detailedRules + '\nEjemplos de uso:\n' + fewShot + '\n' + CONTEXT_BLOCK({
      availableDirsText,
      memory,
      platform,
      trimmedMemory
