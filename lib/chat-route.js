    ...
    try {
      ...
      if (result.type === 'command' && typeof result.command === 'string') {
        ...
        if (AUTO_EXEC_READONLY && isReadOnlyCommand(command)) {
          ...
          if (!execution.ok) {
            const errMsg =
              execution.error === 'shell_operator_not_supported'
                ? 'No puedo ejecutar ese comando directamente. Inténtalo de otra forma.'
                : `El comando falló: ${execution.stderr || execution.error || 'error desconocido'}`;
            return finish(
              {
                type: 'reply',
                message: errMsg
