...
import { AUTO_EXEC_READONLY, AUTO_SUMMARIZE_READS, EXEC_WORKDIR, STRICT_GROUNDED_FS } from './config.js';
import {
  buildVoiceSummary,
...
import { listWorkdirEntries, runInternalInspection, runInternalListing } from './filesystem.js';
import { buildDeterministicInspectionSummary, buildGroundingCommand, buildListingSummary } from './grounding.js';
import {
  executeCommand,
  isBlockedCommand,
  isReadOnlyCommand,
...
export function createChatHandler() {
  return async function chatHandler(req, res) {
    try {
      ...
      // Slot filling — resolve anaphoric references before intent detection
      const { text: filledMessage, subs: slotSubs } = fillSlots(lastUserMessage, memory);
      if (slotSubs.length > 0) {
        console.log('[slots]', slotSubs);
      }
      const fsIntent = isFilesystemIntent(filledMessage);
      const hasEvidence = hasRecentExecutionEvidence(history);
      const listingIntent = detectListingIntent(filledMessage);
      const summaryIntent = detectSummaryIntent(filledMessage);
      const workdirIntent = detectWorkdirIntent(filledMessage);
      const longSummaryRequested = detectLongSummaryRequest(filledMessage);
      const declarationIntent = detectTargetDeclarationIntent(filledMessage);
      const correctionIntent = detectCorrectionIntent(filledMessage);
      ...
      const extractedTarget = extractTargetFromText(filledMessage);
      let resolvedTargetFromContext = null;
      if ([...detectedIntents].some((intent) => FS_INTENTS.has(intent))) {
        resolvedTargetFromContext = await resolveTargetFromContext({ userText: filledMessage, extractedTarget, memory });
        if (!resolvedTargetFromContext && referencesPreviousTarget(filledMessage) && memory.lastTarget && memory.lastTarget !== '.') {
          resolvedTargetFromContext = memory.lastTarget;
        }
      }
      const ordinalIndex = getOrdinalIndexFromText(filledMessage, memory.lastListing.length);
      const ordinalTarget = ordinalIndex !== null && memory.lastListing[ordinalIndex] ? memory.lastListing[ordinalIndex] : null;
      const suggestionOrdinalIndex = getSuggestionOrdinalIndex(filledMessage, memory.lastSuggestions.length);
      ...
      if (workdirIntent) {
        memory.lastAction = 'replied';
        return finish(
          {
            type: 'reply',
            message: EXEC_WORKDIR
          },
          'workdir_reply'
        );
      }
      ...
      if (suggestionTarget) {
        ...
        if (memory.pendingIntent === 'summary') {
          const inspectionTarget = memory.lastTarget || '.';
          const execution = await runInternalInspection({ target: inspectionTarget, includeHidden: true });
          memory.lastAction = 'inspected';
          memory.lastResult = execution.ok ? 'ok' : 'error';
          ...
          return streamDeferredSummary(
            ...
            () => summarizeExecution(filledMessage, execution, { target: inspectionTarget }),
      ...
      if (
        ordinalTarget &&
        (summaryIntent || correctionIntent || referencesPreviousTarget(filledMessage) || isOrdinalSelectionUtterance(filledMessage))
      ) {
      ...
      if (declarationIntent) {
        ...
        const suggestions = await suggestTargets(filledMessage, memory);
      ...
      if (summaryIntent) {
        ...
        const inspectionTarget = resolvedTargetFromContext || memory.lastTarget || '.';
        const execution = await runInternalInspection({ target: inspectionTarget, includeHidden: true });
        if (execution.ok) {
          memory.lastTarget = inspectionTarget;
          memory.notes = [`Ultimo objetivo: ${memory.lastTarget}`];
        }
        memory.lastAction = 'inspected';
        memory.lastResult = execution.ok ? 'ok' : 'error';
      ...
      if (listingIntent) {
        const resolvedTarget = resolvedTargetFromContext || memory.lastTarget || '.';
        const includeHidden = wantsHiddenEntries(filledMessage);
      ...
      const now = Date.now();
      if (!Array.isArray(memory.cachedDirs) || now - Number(memory.cachedDirsAt || 0) > 30_000) {
        memory.cachedDirs = await listWorkdirEntries();
        memory.cachedDirsAt = now;
      }
      const availableDirs = memory.cachedDirs;

      const memoryContext = memory.notes.join('\n');
      const result = await askModel(history, filledMessage, memoryContext, (delta) => {
        sendSse({ type: 'token', delta });
      }, {
        resolvedTarget: resolvedTargetFromContext,
        memory,
        availableDirs
      });
      ...
      if (STRICT_GROUNDED_FS && fsIntent && !hasEvidence && result.type !== 'command') {
        const fallbackTarget = resolvedTargetFromContext || memory.lastTarget || '.';
        if (AUTO_EXEC_READONLY) {
          const execution = await runInternalInspection({ target: fallbackTarget, includeHidden: true });
          const deterministicSummary = buildDeterministicInspectionSummary(fallbackTarget, execution);
          memory.lastTarget = fallbackTarget;
          memory.lastAction = 'inspected';
          memory.lastResult = execution.ok ? 'ok' : 'error';
          if (deterministicSummary && !deterministicSummary.includes('\n')) {
            return finish(
              {
                type: 'reply',
                message: deterministicSummary
              },
              'grounded_fs_deterministic_reply'
            );
          }
          return streamDeferredSummary(
            {
            ...
            () => summarizeExecution(filledMessage, execution),
            'grounded_fs_auto_execution'
          );
        }
        const groundingCommand = await buildGroundingCommand(filledMessage, resolvedTargetFromContext);
        return finish(
          prepareCommandResponse(
            groundingCommand,
      ...
      if (result.type === 'command' && typeof result.command === 'string') {
        ...
        if (AUTO_EXEC_READONLY && isReadOnlyCommand(command)) {
          ...
          if (!execution.ok) {
            if (execution.error === 'shell_operator_not_supported') {
              return finish(
                {
                  type: 'reply',
                  message: 'No puedo ejecutar ese comando directamente. Inténtalo de otra forma.'
                },
                'command_error'
              );
            }

            const retryContext = `El comando "${execution.command}" falló con error: ${execution.stderr || execution.error}. ` +
              `Sugiere un comando alternativo para: ${filledMessage}`;
            ...
            try {
              const retryResult = await askModel(
                ...
                retryContext,
                memoryContext,
                (delta) => sendSse({ type: 'token', delta }),
                { resolvedTarget: resolvedTargetFromContext, memory, availableDirs }
              );

              const retryCommand = String(retryResult?.command || '').trim();
              if (retryResult?.type === 'command' && retryCommand) {
                if (retryCommand.toLowerCase() === command.toLowerCase()) {
                  return finish(
                    {
                      type: 'reply',
                      message: `El comando falló: ${execution.stderr || execution.error || 'error desconocido'}`
                    },
                    'command_error_same_retry'
                  );
                }
                return finish(
                  prepareCommandResponse(retryCommand, 'El primer intento falló. ¿Ejecuto esto en su lugar?'),
                  'retry_command'
                );
              }
              ...
              return finish(
                {
                  type: 'reply',
                  message: retryResult?.message || `El comando falló: ${execution.stderr || execution.error || 'error desconocido'}`
                },
                'command_error_reply'
              );
            } catch {
          ...
          const extracted = extractTargetFromText(filledMessage);
          const resolvedTarget = await resolveTargetInWorkdir(extracted, filledMessage);
          if (resolvedTarget) memory.lastTarget = resolvedTarget;
          memory.lastSuggestions = [];
          memory.notes = [`Ultimo objetivo: ${memory.lastTarget}`];
          memory.lastTarget = resolvedTarget || memory.lastTarget || '.';
          memory.lastAction = 'executed';
